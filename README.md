...
- **热词管理** — 上传简历/JD/技术文档，AI 自动提取专业术语并联想扩展，手动微调后一键应用到 ASR；11 个内置技术方向预设模板（前端/后端/数据科学/AI/产品/设计/运维/安全/算法/移动端/金融），统一配置本地和云端热词，英文专业词汇识别准确率显著提升
- **知识库复用** — AI 回答页面可一键从知识库选择已上传的简历、岗位 JD 等资料，一次上传多处复用
...
- AI 自动提取专业术语并联想扩展更多相关词汇，手动微调后一键应用到语音识别
- 也可选择 11 个内置技术方向预设模板快速加载
- 在「AI 回答」页面点击「知识库」按钮，可一键加载已保存的文档作为参考资料
...
管理热词提取、预设模板和知识库文档。支持从简历/JD 自动提取专业术语（LLM 驱动），AI 联想扩展更多相关词汇，手动增删微调后一键应用到 ASR。内置 11 个技术方向预设模板，双格式输出（本地 sherpa-onnx 格式 + 云端 DashScope 格式）。知识库支持文档上传和持久化，AI 回答模块可一键选择已保存文档作为参考资料。
...
- **v2026.06.16** - 新增热词管理（上传简历/JD/技术文档，AI 自动提取专业术语并联想扩展，手动微调后一键应用到 ASR，11 个内置技术方向预设模板）；新增知识库复用（AI 回答页面一键选择已上传资料）；DashScope 热词表 API 集成（vocabulary_id 方式，英文专业词汇识别率显著提升）；支持 Fun-ASR/Qwen-ASR/Paraformer 三种云端模型切换；修复 DashScope 云端 ASR 实时识别无输出和发送后重复显示问题；WebSocket 替换为标准 ws 库解决中文乱码
