...
- **v2026.06.12** - 内置语音识别模型，安装即用；前端界面深度优化，支持用户 DIY 主题/颜色/透明度；5 种本地语音模型可选（GitHub 镜像加速）；双重验证识别（在线流式 + 离线修正）；首次引导配置向导；检查更新功能；许可证激活系统；修复滚动卡死、ASR 重复、AI 回答延迟等问题；两者模式并行、触摸屏支持、自定义安装目录
