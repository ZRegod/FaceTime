...
- **模型一键下载** — 5 种本地语音模型可选（极小中文 20MB / 轻量中文 71MB / 中文增强 59MB / 中英双语 331MB / 带标点 128MB），支持 GitHub 镜像加速
- **双重验证识别** — 在线流式识别 + 离线修正两遍校验，内置 VAD 语音检测和热词库（技术面试专用）
...
│       ├── sherpaModelManager.ts # Sherpa-ONNX 模型下载管理（5 种模型 + 镜像加速）
...
- **v2026.06.12** - 前端界面深度优化，支持用户 DIY 主题/颜色/透明度；新增 5 种本地语音模型一键下载（GitHub 镜像加速）；双重验证识别（在线流式 + 离线修正）；修复滚动卡死、ASR 重复、AI 回答延迟等问题；新增两者模式并行、触摸屏支持、自定义安装目录
